...
def seqNet(df_roi, fid_roi, v_roi, lva_roi, lha_roi, l_roi):
    ...
    for i in range(CrossValDurchläufe):
        ...
        #model.add(Dense(64,  kernel_regularizer=tf.keras.regularizers.l2(0.001), activation="tanh"))
        #model.add(Dense(128, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation="tanh"))
        ...
        lossVec[i]          = H.history["train_loss"][-1]
    ...
    #Zusammenfassung
    model.summary()    
    print("train_loss der "+str(numberOfEpochs)+" Epochen: "+ str(lossVec) + "  ~~~~~~~> "+str(np.mean(lossVec)))
    print("val_loss der "+str(numberOfEpochs)+" Epochen: "+ str(val_lossVec) + "  ~~~~~~~> "+str(np.mean(val_lossVec)))
    print("train_accuracy der "+str(numberOfEpochs)+" Epochen: "+ str(accuracyVec) + "  ~~~~~~~> "+str(np.mean(accuracyVec)))
    print("val_accuracy der "+str(numberOfEpochs)+" Epochen: "+ str(val_accuracyVec) + "  ~~~~~~~> "+str(np.mean(val_accuracyVec)))
    print("train_precision der "+str(numberOfEpochs)+" Epochen: "+ str(precisionVec) + "  ~~~~~~~> "+str(np.mean(precisionVec)))
    print("val_precision der "+str(numberOfEpochs)+" Epochen: "+ str(val_precisionVec) + "  ~~~~~~~> "+str(np.mean(val_precisionVec)))
    print("train_recall der "+str(numberOfEpochs)+" Epochen: "+ str(recallVec) + "  ~~~~~~~> "+str(np.mean(recallVec)))
